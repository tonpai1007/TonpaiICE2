// src/errorMessages.js - Centralized error messages with helpful guidance

const ERROR_MESSAGES = {
  // Parsing errors
  CANNOT_PARSE: {
    message: '❌ ไม่เข้าใจคำสั่ง',
    help: [
      '💡 ตัวอย่างที่ถูกต้อง:',
      '• "เจ้แอน สั่ง โค้ก 30 จำนวน 5"',
      '• "น้ำแข็งหลอด 20 2 ถุง"',
      '• "เติม น้ำแข็ง 20 50"',
      '',
      'พิมพ์ "help" เพื่อดูคู่มือฉบับเต็ม'
    ]
  },
  
  AMBIGUOUS_PRODUCT: (productName, matches) => ({
    message: `🤔 พบสินค้า "${productName}" หลายรายการ`,
    help: [
      'กรุณาเลือกแบบที่ต้องการ:',
      '',
      ...matches.slice(0, 5).map((m, i) => 
        `${i + 1}. ${m.item} - ${m.price}฿ (เหลือ ${m.stock} ${m.unit})`
      ),
      '',
      '💡 วิธีระบุให้ชัดเจน:',
      `พิมพ์: "${productName} [ราคา] [จำนวน]"`,
      `ตัวอย่าง: "${productName} ${matches[0].price} 2"`
    ]
  }),
  
  // Stock errors
  OUT_OF_STOCK: (item, requested, available) => ({
    message: `❌ สต็อกไม่พอ: ${item}`,
    help: [
      `📊 สถานะ:`,
      `• มีอยู่: ${available} ${item.unit || 'ชิ้น'}`,
      `• ต้องการ: ${requested} ${item.unit || 'ชิ้น'}`,
      `• ขาด: ${requested - available} ${item.unit || 'ชิ้น'}`,
      '',
      '💡 ทางเลือก:',
      '• สั่งน้อยลง',
      '• ติดต่อแอดมินเพื่อเติมสต็อก'
    ]
  }),
  
  PRODUCT_NOT_FOUND: (productName) => ({
    message: `❌ ไม่พบสินค้า: ${productName}`,
    help: [
      '💡 ทำอย่างไร:',
      '• ตรวจสอบชื่อสินค้า (อาจพิมพ์ผิด)',
      '• พิมพ์ "สต็อก" เพื่อดูรายการสินค้าทั้งหมด',
      '• ติดต่อแอดมินหากต้องการเพิ่มสินค้าใหม่'
    ]
  }),
  
  // Order errors
  ORDER_NOT_FOUND: (orderNo) => ({
    message: `❌ ไม่พบออเดอร์ #${orderNo}`,
    help: [
      '💡 กรุณาตรวจสอบ:',
      '• เลขออเดอร์ถูกต้องหรือไม่',
      '• พิมพ์ "ออเดอร์ล่าสุด" เพื่อดูออเดอร์ล่าสุด',
      '• ถ้าไม่ระบุเลข ระบบจะใช้ออเดอร์ล่าสุดอัตโนมัติ'
    ]
  }),
  
  INVALID_QUANTITY: (quantity) => ({
    message: `❌ จำนวนไม่ถูกต้อง: ${quantity}`,
    help: [
      '📋 ข้อกำหนด:',
      '• จำนวนต้องเป็นตัวเลขบวก',
      '• ระหว่าง 1 - 10,000',
      '',
      '💡 ตัวอย่าง:',
      '• "โค้ก 30 จำนวน 5" ✅',
      '• "โค้ก 30 -5" ❌',
      '• "โค้ก 30 99999" ❌'
    ]
  }),
  
  // Permission errors
  NO_PERMISSION: (action) => ({
    message: `🔒 คุณไม่มีสิทธิ์: ${action}`,
    help: [
      '💡 ทำอย่างไร:',
      '• ติดต่อแอดมินเพื่อขอสิทธิ์',
      '• พิมพ์ "help" เพื่อดูคำสั่งที่ใช้ได้'
    ]
  }),
  
  // System errors
  SYSTEM_ERROR: {
    message: '❌ เกิดข้อผิดพลาดของระบบ',
    help: [
      '💡 กรุณา:',
      '• ลองใหม่อีกครั้งในอีกสักครู่',
      '• ถ้ายังไม่ได้ ติดต่อแอดมิน',
      '• ระบบจะบันทึกข้อผิดพลาดไว้แล้ว'
    ]
  },
  
  RATE_LIMIT: (resetIn) => ({
    message: '⏰ คุณส่งข้อความเร็วเกินไป',
    help: [
      `💡 กรุณารอ ${resetIn} วินาที`,
      '',
      'ระบบจำกัดความถี่เพื่อป้องกันการใช้งานที่ผิดปกติ'
    ]
  }),
  
  // Voice errors
  VOICE_TRANSCRIPTION_FAILED: {
    message: '❌ ไม่สามารถฟังเสียงได้ชัดเจน',
    help: [
      '💡 กรุณา:',
      '• พูดช้าและชัดเจน',
      '• ลดเสียงรบกวนรอบข้าง',
      '• อยู่ในที่เงียบ',
      '• ถือโทรศัพท์ใกล้ปาก',
      '',
      'หรือพิมพ์ข้อความแทนก็ได้ค่ะ'
    ]
  },
  
  VOICE_NOT_SUPPORTED: {
    message: '❌ ระบบเสียงยังไม่พร้อมใช้งาน',
    help: [
      '💡 ในขณะนี้:',
      '• กรุณาใช้ข้อความแทน',
      '• แอดมินกำลังแก้ไขระบบ',
      '• จะแจ้งเมื่อพร้อมใช้งาน'
    ]
  }
};

function formatError(errorKey, ...args) {
  let error = ERROR_MESSAGES[errorKey];
  
  if (typeof error === 'function') {
    error = error(...args);
  }
  
  if (!error) {
    error = ERROR_MESSAGES.SYSTEM_ERROR;
  }
  
  let message = error.message;
  
  if (error.help && error.help.length > 0) {
    message += '\n\n' + error.help.join('\n');
  }
  
  return message;
}

// Quick error responses
const QuickErrors = {
  cannotParse: () => formatError('CANNOT_PARSE'),
  ambiguousProduct: (name, matches) => formatError('AMBIGUOUS_PRODUCT', name, matches),
  outOfStock: (item, requested, available) => formatError('OUT_OF_STOCK', item, requested, available),
  productNotFound: (name) => formatError('PRODUCT_NOT_FOUND', name),
  orderNotFound: (orderNo) => formatError('ORDER_NOT_FOUND', orderNo),
  invalidQuantity: (qty) => formatError('INVALID_QUANTITY', qty),
  noPermission: (action) => formatError('NO_PERMISSION', action),
  systemError: () => formatError('SYSTEM_ERROR'),
  rateLimit: (seconds) => formatError('RATE_LIMIT', seconds),
  voiceError: () => formatError('VOICE_TRANSCRIPTION_FAILED'),
  voiceNotSupported: () => formatError('VOICE_NOT_SUPPORTED')
};

module.exports = {
  ERROR_MESSAGES,
  formatError,
  QuickErrors
};